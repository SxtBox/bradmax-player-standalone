...
class BradmaxPlayer {
    ...
    private function handleRequests() {
        ...
        if (isset($_SESSION['bradmax_upload_info'])) {
            $this->uploadInfo = $_SESSION['bradmax_upload_info'];
            unset($_SESSION['bradmax_upload_info']);
        }
        ...
        if ($postHandled) {
            $_SESSION['bradmax_upload_info'] = $this->uploadInfo;
...
<head>
	...
	<script src="<?php echo APP_URL . ltrim(self::CUSTOMIZED_PLAYER_FILE, '/'); ?>" defer></script>
	<?php endif; ?>
	<script src="<?php echo APP_URL; ?>assets/js/stream_player.js" defer></script>
</head>
<body class="neon-bg">
    <div class="stars"></div>
    ...
    <div class="container">
        <div class="neon-card">
            <div class="neon-header">
                <div>
                    <h1>🎬 Bradmax Player PRO</h1>
                    <p class="subtitle">Professional Video Player Manager</p>
                </div>
                <span class="version-badge">v<?php echo APP_VERSION; ?></span>
            </div>
            <nav class="tab-nav" aria-label="Player sections">
                <button type="button" class="tab-button is-active" data-tab-target="stream-tab">▶️ Stream Player</button>
                <button type="button" class="tab-button" data-tab-target="upload-tab">📤 Upload Player</button>
            </nav>
            <!-- Messages -->
...
<?php endif; ?>
            <section id="stream-tab" class="tab-panel is-active">
            <!-- Stream Player Panel -->
            <div class="stream-section neon-glow-cyan">
                <h2>▶️ Play a Stream</h2>
                <p class="section-description">Enter a stream URL and press Play. HTML5 is used by default; an uploaded Bradmax player is used automatically.</p>
                <form id="stream-form" class="stream-form">
                    <div class="form-group stream-url-group">
                    ...
                    <div class="stream-actions">
                        <button type="submit" class="btn btn-primary neon-glow-cyan">▶️ Play Stream</button>
                        <button type="button" id="stream-clear" class="btn btn-secondary">Clear</button>
                    </div>
                </form>
                <div id="stream-status" class="stream-status" role="status" aria-live="polite"></div>
                <div id="stream-player" class="stream-player" data-bradmax-enabled="<?php echo $this->customPlayerExists ? 'true' : 'false'; ?>">
                    <video id="stream-video" controls playsinline preload="metadata">
                        Your browser does not support HTML5 video.
                    </video>
                </div>
            </div>
            </section>
            <section id="upload-tab" class="tab-panel">
<!-- Current Player Info -->
<?php if ($this->customPlayerExists): ?>
                <div class="player-info neon-glow-cyan">
                    ...
                    <form method="POST" class="remove-form">
                        <button type="submit" name="remove_player" class="btn btn-danger neon-glow-red">
            ...
            <!-- Footer -->
            <div class="footer">
                <p>Bradmax Player PRO v<?php echo BRADMAX_PLAYER_VERSION; ?> | Standalone Edition</p>
